        ...
        for line in infile:
            ...
            if book_chapter_match:
                if current_verse:
                    ...
                    if verse_num:
                        rows.append({
                            "Book": current_book,
                            "Verse": f"{current_chapter}:{verse_num.group(1)}",
            ...
            if re.match(r"^\d+", no_refs):
                ...
                if m:
                    ...
                    rows.append({"Book": current_book, "Verse": f"{current_chapter}:{n1}", "Sentence": verse_text})
                    rows.append({"Book": current_book, "Verse": f"{current_chapter}:{n2}", "Sentence": verse_text})
                    current_verse = ""
                else:
                    # Normal single verse with space
